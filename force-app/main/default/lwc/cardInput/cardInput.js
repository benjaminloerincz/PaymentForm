...
export default class CardInput extends LightningElement {
    ...
    handleNameInput(event) {
        ...
        this.cardHolderNameValid = this.getIsValid(
            this.cardHolderName,
            'cardHolderName'
        );
    ...
    showFeedback() {
        if (!this.cardNumberValid && this.cardNumberTouched) {
            //show error label
            this.template
                .querySelectorAll('.cardNumberError')[0]
        ...
        if (!this.cardHolderNameValid && this.cardHolderNameTouched) {
            //show error label
            this.template
                .querySelectorAll('.cardNameError')[0]
                .classList.remove('slds-hide');
            this.template
                .querySelectorAll('.cardNameFormElement')[0]
                .classList.add('slds-has-error');
        } else {
            this.template
                .querySelectorAll('.cardNameError')[0]
        ...
        if (!this.cardExpiryValid && this.cardExpiryTouched) {
            //show error label
            this.template
                .querySelectorAll('.cardExpiryError')[0]
                .classList.remove('slds-hide');
            this.template
        ...
        if (!this.cardCVCValid && this.cardCVCTouched) {
            //show error label
            this.template
                .querySelectorAll('.cardCVVError')[0]
                .classList.remove('slds-hide');
            this.template
                .querySelectorAll('.cardCVVFormElement')[0]
                .classList.add('slds-has-error');
        } else {
            this.template
                .querySelectorAll('.cardCVVError')[0]
