({
    ...
    checkIfPaymentValid: function(component) {
        var paymentMethod = component.get('v.paymentMethod');
        var cardValid = component.get('v.cardValid');
        var paymentValid = false;

        if (paymentMethod == 'Check') {
            var checkNumber = component.get('v.checkNumber');
    ...
    checkIfAddressValid: function(component) {
        return component.get('v.addressValid');
    },

    displayError: function(component, errorTitle, error) {
        return new Promise((resolve, reject) => {
            component.set('v.showSpinner', false);
            this.notify(
                component,
                "The payment you submitted could not be processed at this time. If you think that's in error, try resubmitting or using another form of payment.",
                'error',
                'dismissable'
            );
    ...
    showSpinner: function(component, spinnerClass) {
        component.set(
            'v.spinnerClass',
            $A.util.isEmpty(spinnerClass) ? 'loading' : spinnerClass
        );
    ...
    loadStateOptions: function(component) {
        var states = [
            { value: '', label: 'Select a State', selected: false },
    ...
    loadCountryOptions: function(component) {
        var rawCountries = [
            { value: '', label: 'Select a Country', selected: false },
